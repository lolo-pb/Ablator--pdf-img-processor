...
- `packages/extraction`: xAI Grok extraction pipeline and mock fallback
...
If `XAI_API_KEY` is not set, extraction falls back to a deterministic mock provider so the review/export flow still works.
